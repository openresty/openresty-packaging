...
Source1:    openresty-postgresql16.init
Source2:    openresty-postgresql16.service
Source3:    postgresql16-check-db-dir
...
Summary:    Development files for openresty-postgresql16
Group:      Applications/System
Requires:   %{name} = %{version}-%{release}

%description devel
Provides C header and static library for the openresty-postgresql16 package.
...
%{__install} -p -m 0755 %{SOURCE1} $RPM_BUILD_ROOT/etc/init.d/openresty-postgresql16
%{__install} -p -m 0755 %{SOURCE2} $RPM_BUILD_ROOT/%{pgprefix}/share/systemd/openresty-postgresql16.service
...
if [ -d "/etc/systemd/system" ]; then
    if [ -L /etc/systemd/system/openresty-postgresql16.service ]; then
        rm -f /etc/systemd/system/openresty-postgresql16.service
    fi
...
if [ $1 = 0 ]; then
    ...
    if [ -n "$(command -v chkconfig)" ]; then
        $(command -v chkconfig) --del openresty-postgresql16
    fi
fi
...
if [ $1 == 1 ];then
    ...
    mkdir -p /var/postgres16
    mkdir -p /var/postgres16/{data,scripts,conf,log}
    chown -R postgres:postgres /var/postgres16
fi
...
%{pgprefix}/lib/pgxs/*
%{pgprefix}/share/*
/etc/init.d/openresty-postgresql16

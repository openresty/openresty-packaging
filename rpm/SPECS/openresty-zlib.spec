...
export LDFLAGS="$LDFLAGS -Wl,-rpath,%{zlib_prefix}/%{_lib} -Wl,-z,relro -Wl,-z,now"
./configure \
  --libdir=%{zlib_prefix}/%{_lib} \
...
rm -f  %{buildroot}/%{zlib_prefix}/%{_lib}/*.la
rm -f  %{buildroot}/%{zlib_prefix}/%{_lib}/*.a
rm -rf %{buildroot}/%{zlib_prefix}/%{_lib}/pkgconfig

%files
%defattr(-,root,root,-)
%attr(0755,root,root) %{zlib_prefix}/%{_lib}/libz.so*
